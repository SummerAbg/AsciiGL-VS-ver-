...
#ifdef AsciiAPI
#define AsciiAPI __declspec(dllexport)
#else
#define AsciiAPI __declspec(dllimport)
#endif
 ...
 *  @retval        - 因变量
 */
AsciiAPI double getY(Coord2d coordA, Coord2d coordB, double x);
 ...
 *  @retval             - 块个数
 */
AsciiAPI int getBlockNumber(int strSize, int block_length);
...
template <typename T>
AsciiAPI inline bool isExist(const std::vector<T> &vec, const T &element) {
