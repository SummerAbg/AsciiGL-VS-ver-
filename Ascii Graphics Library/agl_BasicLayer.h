 ...
 *
 */
class AsciiAPI AsciiBasicLayer : public AsciiBasicCanvas {
public:
  /**
   ...
   */
  AsciiBasicLayer(const AsciiBasicCanvas &canvas, const Coord2d &coord,
                  const std::string &name, bool display_state = true);
   ...
   *  @param coord - 指定坐标
   */
  void setCoordinate(const Coord2d &coord) { this->coord = coord; }
   ...
   *  @param coord - 指定坐标
   */
  void setCenterCoordinate(const Coord2d &coord);

  /**
   *  @brief  获取图层中画布的中心坐标
   *  @retval  - 中心坐标
   */
  Coord2d getCenterCoordinate() const { return center_coord; }
   ...
   *  @retval  - 显示状态的布尔值
   */
  bool isDisplay() const { return display_state; }
